...
Preview and production deploys use Neon branches and Cloudflare Workers:

- `deploy-preview.yml` creates `preview/pr-<number>` in Neon and deploys
  `smithery-pr-<number>` to Workers. A single pull request comment is updated with
  the latest deployment URL or failure status.
- `cleanup-preview.yml` deletes both resources when the pull request closes or merges.
- `deploy-production.yml` resolves Neon's primary branch and updates the stable
  `smithery` Worker on every push to `main`.

The workflows require `NEON_API_KEY`, `NEON_PROJECT_ID`, `CLOUDFLARE_API_TOKEN`,
and `CLOUDFLARE_ACCOUNT_ID`. Preview deploys run only for branches in this
repository.
