...
function ApiWorkbench({
  ...
  useHotkey(
    ...
    { enabled: Boolean(onClearAuth) && !authPending, ignoreInputs: false },
  )
  ...
  const serverHints = manyServers
    ...
    : []
  const clearAuthHint = onClearAuth
      ...
      <div className="shrink-0 border-b border-rule">
        <div className="flex flex-wrap items-center gap-3 px-3 py-2 md:px-4">
          ...
          {onClearAuth ? (
            <button
              type="button"
              className="ml-auto inline-flex items-center gap-2 border-0 bg-ink/10 px-3 py-1.5 text-sm font-medium text-ink shadow-none outline-none hover:bg-ink/15"
