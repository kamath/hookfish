...
- `deploy-preview.yml` creates `preview/pr-<number>` in Neon, migrates it, and deploys
  `hookfish-pr-<number>` to Workers. The Neon URL is uploaded with the Worker version as
  an encrypted secret. A single pull request comment is updated with the
...
- `deploy-production.yml` migrates Neon's primary branch and updates the stable `hookfish`
